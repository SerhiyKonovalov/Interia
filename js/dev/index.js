import { b as bodyLock, a as bodyUnlock, c as bodyLockStatus, d as bodyLockToggle, g as gotoBlock, u as uniqArray, e as getHash, i as isMobile } from "./common.min.js";
(function polyfill() {
    ...
    new Swiper(".hero__slider", {
      ...
      pagination: {
        ...
        renderBullet: function(index, className) {
          return `<div data-slide="${index}" class="${className} effect-3d"><span>${index < 10 ? "0" + (index + 1) : index + 1}</span><span>${index < 10 ? "0" + (index + 1) : index + 1}</span></div>`;
        }
      },
      // Скроллбар
      /*
...
function customCursor() {
  ...
  if (!wrapper || isMobile.any()) return;
  const cursor = document.createElement("div");
  cursor.className = "fls-cursor";
  ...
  const shadowHalf = shadow.offsetWidth / 2;
  let targetX = 0;
  let targetY = 0;
  let pointerX = 0;
  let pointerY = 0;
  ...
  let shadowY = 0;
  function animate() {
    pointerX += (targetX - pointerX) * 0.25;
    pointerY += (targetY - pointerY) * 0.25;
    shadowX += (targetX - shadowX) * 0.12;
    shadowY += (targetY - shadowY) * 0.12;
    pointer.style.transform = `translate3d(
			${pointerX - pointerHalf}px,
			${pointerY - pointerHalf}px,
			0
		)`;
    shadow.style.transform = `translate3d(
			${shadowX - shadowHalf}px,
			${shadowY - shadowHalf}px,
			0
		)`;
